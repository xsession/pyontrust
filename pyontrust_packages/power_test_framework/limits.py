...
A :class:`TestSpec` defines measurable expectations for a test run.
After capture, the :func:`evaluate` function compares actual measurements
against the spec and returns a :class:`TestVerdict`.

Example JSON in a profile::

    "limits": {
      "steps": {
        "sleep": {
          "avg_current_a": {"max": 10e-6},
          "max_current_a": {"max": 50e-6}
        },
        "active": {
          "avg_current_a": {"min": 1e-3, "max": 20e-3}
        }
      },
      "overall": {
        "energy_j": {"max": 0.1},
...
class Verdict(enum.Enum):
    ...
    PASS = "PASS"
    FAIL = "FAIL"
    WARN = "WARN"       # within warn threshold but not failed
    SKIP = "SKIP"       # no limits defined
    ERROR = "ERROR"     # could not evaluate (missing data)
...
class Limit:
    ...
    Optional ``warn_min`` / ``warn_max`` trigger :attr:`Verdict.WARN`.
    """
    min: float | None = None
    max: float | None = None
    ...
    def check(self, value: float) -> Verdict:
        """Evaluate *value* against this limit."""
        ...
        for metric, limit in self.limits.items():
            if metric not in measurements:
                results.append(LimitResult(
                    metric=metric, value=float("nan"), limit=limit,
                    verdict=Verdict.ERROR, step_name=step_name,
                ))
                continue
            val = measurements[metric]
            verdict = limit.check(val)
            results.append(LimitResult(
                metric=metric, value=val, limit=limit,
                verdict=verdict, step_name=step_name,
            ))
        return results
...
class TestSpec:
    ...
    step_limits : dict[str, StepLimits]
        Limits per named step (keyed by step name).
    overall_limits : StepLimits
        Limits applied to the overall test summary.
    """
    ...
    def from_dict(cls, raw: dict[str, Any]) -> TestSpec:
        """Parse from the ``"limits"`` section of a profile JSON."""
        step_limits: dict[str, StepLimits] = {}
...
def evaluate(
    ...
    """Evaluate measurements against a :class:`TestSpec`.

    Parameters
    ----------
    spec : TestSpec
        The pass/fail specification.
    overall_summary : dict[str, float]
        Overall measurements (from ``PowerSummary.__dict__``).
    step_summaries : dict[str, dict[str, float]] | None
        Per-step measurements keyed by step name.
