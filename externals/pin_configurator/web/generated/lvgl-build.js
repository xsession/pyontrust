...
(() => {
    ...
    function transitionInfo(transition) {
        return window.lvglTransition ? window.lvglTransition(transition) : { anim: "LV_SCR_LOAD_ANIM_MOVE_LEFT" };
    }
    function lvglStateMask(style) {
        if (!style || style.state === "default")
            return style?.part || "LV_PART_MAIN";
        const stateMap = {
    ...
    function buildSharedStyleMeta(state) {
    ...
    function emitStyleSetters(lines, styleSymbol, values) {
        if (values.bg) {
    ...
    function buildHooksHeader(state) {
        const lines = [
            "// Generated by Zephyr Pin Configurator - LVGL Layout Editor",
            "// Generated file: safe to overwrite when layout changes.",
            "#pragma once",
    ...
    function buildHooksSource(state) {
        const lines = [
            "// Generated by Zephyr Pin Configurator - LVGL Layout Editor",
    ...
    function buildCode(state) {
        ...
        const screenMeta = state.screens.map((screen) => ({
            screen,
        ...
        const metaById = new Map(screenMeta.map((item) => [item.screen.id, item]));
        const sharedStyles = buildSharedStyleMeta(state);
        const styleById = new Map(sharedStyles.map((style) => [style.id, style]));
        ...
        widgetCallbacks.forEach((item) => {
            if (!item.target || seenCallbacks.has(item.callbackName))
                return;
            seenCallbacks.add(item.callbackName);
            uniqueCallbacks.push(item);
        });
        widgetCallbacks.forEach((item) => {
            if (seenCallbacks.has(item.callbackName))
                return;
            seenCallbacks.add(item.callbackName);
            uniqueCallbacks.push(item);
        });
    ...
    function buildIntegrationGuide(state, issues) {
