...
(() => {
    ...
    function normalizeStyleRefs(value) {
        return Array.isArray(value)
            ? value.map((entry) => String(entry || "").trim()).filter(Boolean)
            : [];
    }
    function normalizeNode(node = {}, isScreen) {
    ...
    function normalizeState(rawState, helpers) {
        ...
        if (Array.isArray(legacyNodes) && legacyNodes.length) {
            ...
            state = {
                ...
                screens: [{
                        ...root,
                        nodes: helpers.cloneJson(legacyNodes.slice(1)).map((node) => normalizeNode(node, false)),
                    }],
            };
        }
    ...
    function codegenSymbol(name, fallback = "node") {
    ...
    function validateState(state) {
        ...
        const screenIdList = new Set();
        const screenNames = new Map();
        const sharedStyleIds = new Set();
        const sharedStyleNames = new Map();
        const sharedStyleUsage = new Map();
        const widgetIds = new Set();
        const screenInboundTargets = new Set();
        ...
        const normalizeStylePart = (value) => {
        ...
        const normalizeStyleState = (value) => String(value || "").trim().toLowerCase();
        const nodeLabel = (node) => String(node.name || node.id || node.type || "Widget").trim() || "Widget";
        const screenLabel = (screen) => String(screen.name || screen.id || "screen").trim() || "screen";
        const recordGeneratedSymbol = (scope, issueId, label, symbol) => {
        ...
        const validateReservedSymbol = (scope, issueId, label, symbol) => {
    ...
    function buildValidationReport(state, issues) {
        const bySeverity = {
            error: issues.filter((issue) => issue.severity === "error"),
            warning: issues.filter((issue) => issue.severity === "warning"),
            info: issues.filter((issue) => issue.severity === "info"),
        };
        const scopeCounts = issues.reduce((acc, issue) => {
    ...
    function resolveNodeVisual(state, node) {
        ...
        const mergedValues = applicable.reduce((acc, style) => ({
            ...acc,
            ...(style.values || {}),
        }), {});
        return {
