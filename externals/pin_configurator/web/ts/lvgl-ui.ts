// @ts-nocheck
(() => {
  type LayoutSnapshot = LayoutState;
  type LayoutRestoreState = (Partial<LayoutState> & { nodes?: LayoutNode[] }) | null;
  ...
  let dragStartSnapshot: LayoutSnapshot | null = null;
  let widgetClipboard: LayoutNode | null = null;
  let treeFilter = "";
  let styleFilter = "";
  ...
  const MAX_ZOOM = 2.5;
  const ZOOM_STEP = 0.1;
  const canvasView: CanvasViewState = {
    zoom: 1,
    panX: 0,
    panY: 0,
    tool: "select",
  ...
  function escapeNodeLabel(node?: LayoutNode | null): string {
    return window.escapeHtml(window.LvglRegistry?.nodeLabel(node) || node?.name || "widget");
  }

  function serializeStateForHistory(): LayoutSnapshot {
    return window.cloneJson(window.lvglEnsureState());
  }

  function snapshotsEqual(left: LayoutSnapshot, right: LayoutSnapshot): boolean {
    return JSON.stringify(left) === JSON.stringify(right);
  }

  function isLvglTabActive(): boolean {
  ...
  function updateHistoryButtons(): void {
    ...
    if (copyBtn) copyBtn.disabled = selectedNodes.length !== 1;
    if (duplicateBtn) duplicateBtn.disabled = selectedNodes.length !== 1;
    if (pasteBtn) pasteBtn.disabled = !widgetClipboard;
  }

  function selectedIds(state: LayoutState = window.lvglEnsureState()): string[] {
  ...
  function syncSelectionState(state: LayoutState, nextIds: string[], primaryId = ""): void {
  ...
  function selectedNodeEntries(state: LayoutState = window.lvglEnsureState()): LvglNodeLookup[] {
    return selectedIds(state)
      .map(id => window.lvglFindNode(id))
      .filter((entry): entry is LvglNodeLookup => !!entry);
  }

  function selectedWidgetNodes(state: LayoutState = window.lvglEnsureState()): LayoutNode[] {
    return selectedNodeEntries(state)
      .filter(entry => !entry.isScreen)
      .map(entry => entry.node as LayoutNode);
  }

  function nodeIsSelected(state: LayoutState, nodeId: string): boolean {
    return selectedIds(state).includes(nodeId);
  }

  function setSingleSelection(state: LayoutState, nodeId: string): void {
  ...
  function toggleWidgetSelection(state: LayoutState, nodeId: string): void {
    const found = window.lvglFindNode(nodeId);
  ...
  function recordHistorySnapshot(snapshot: LayoutSnapshot | null): void {
    if (!snapshot || historyState.isApplying) return;
  ...
  function applyMutation(mutator: (state: LayoutState) => void, options: MutationOptions = {}): void {
  ...
  function restoreStateInternal(nextState: LayoutRestoreState, options: MutationOptions = {}): void {
    ...
    try {
      ...
      const ids = [];
      window.lvglLayoutState.screens.forEach((screen: ScreenNode) => {
        ids.push(screen.id);
        (screen.nodes || []).forEach((node: LayoutNode) => ids.push(node.id));
      });
  ...
  function undo(): void {
    if (!historyState.undoStack.length) return;
    ...
    if (["ArrowUp", "ArrowDown", "ArrowLeft", "ArrowRight"].includes(event.key)) {
      ...
      applyMutation(() => {
        moveNodes.forEach(target => {
          ...
          if (!found?.node || found.isScreen) return;
          if (event.key === "ArrowUp") found.node.y -= distance;
          if (event.key === "ArrowDown") found.node.y += distance;
          if (event.key === "ArrowLeft") found.node.x -= distance;
          if (event.key === "ArrowRight") found.node.x += distance;
  ...
  function clipboardSourceNode(): LayoutNode | null {
    const node = window.lvglSelectedNode();
    if (!node || node.type === "screen") return null;
    return node as LayoutNode;
  }

  function normalizeNameBase(name: string | undefined, type: string): string {
    const raw = String(name || type || "widget").trim();
    return raw.replace(/_\d+$/, "") || type || "widget";
  }

  function uniqueWidgetName(state: LayoutState, type: string, preferredName?: string): string {
  ...
  function createWidgetClone(
    state: LayoutState,
    sourceNode: LayoutNode | null,
    screen: ScreenNode,
    options: WidgetCloneOptions = {},
  ): LayoutNode | null {
    if (!sourceNode || sourceNode.type === "screen") return null;
  ...
  function pasteClipboard(): boolean {
    ...
    applyMutation((state: LayoutState) => {
  ...
  function duplicateSelectedWidget(): boolean {
    const node = clipboardSourceNode();
    if (!node) return false;
  ...
  function nextStyleId(state: LayoutState): string {
  ...
  function selectedStyle(state: LayoutState): SharedStyle | null {
    return window.LvglModel?.findSharedStyle(state, state.selectedStyleId) || null;
  }

  function ensureSelectedStyle(state) {
    if (state.selectedStyleId && selectedStyle(state)) return selectedStyle(state);
  ...
  function renderValidation() {
    ...
    if (!summaryEl || !listEl || !searchFilterEl || !severityFilterEl || !scopeFilterEl || !applyRenamesBtn || !resetFiltersBtn) return;
  ...
  function updateQuickStyleBar() {
    ...
    if (!bgInput || !colorInput || !radiusInput || !hint) return;
  ...
  function renderStage() {
    ...
    stage.querySelectorAll("[data-lvgl-node]").forEach(nodeEl => {
      nodeEl.addEventListener("click", (event) => {
        ...
        if (canvasView.tool === "hand") return;
        event.stopPropagation();
      ...
      nodeEl.addEventListener("mousedown", (event) => {
        if (state.simulation.running || canvasView.tool === "hand") return;
        const found = window.lvglFindNode(nodeEl.dataset.lvglNode);
        if (!found || found.isScreen) return;
  ...
  function renameSuggestionForIssue(state, issue, found, style) {
    ...
    if (style) {
      ...
      const suggestedName = uniqueNameBySymbol(
        message.includes("reserved C identifier") ? `${style.name || style.id} ui` : `${style.name || style.id}`,
        `style_${style.id || "shared"}`,
        existingSymbols,
      );
      return {
    ...
    if (found.isScreen) {
      if (message.startsWith("Entry hook for ")) {
        ...
        const suggestedName = uniqueNameBySymbol(
          message.includes("reserved C identifier") ? `${currentHook} ui` : currentHook,
          `on_enter_${screenSymbol}`,
          existingSymbols,
        );
        return {
      ...
      const suggestedName = uniqueNameBySymbol(
        message.includes("reserved C identifier") ? `${found.node.name} ui` : found.node.name,
        "screen",
        existingSymbols,
      );
      return {
    ...
    return {
      ...
      apply: () => {
        ...
        if (!current || current.type === "screen") return;
  ...
  function nodeSymbolPreviewMarkup(node, parentScreen, highlightedSymbols = new Set()) {
    ...
    if (node.type === "screen") {
      ...
      return symbolPreviewMarkup(
        "Codegen Preview",
        "These are the identifiers the generator will emit for the selected screen.",
        entries,
        highlightedSymbols,
      );
    }
    ...
    return symbolPreviewMarkup(
      "Codegen Preview",
      "These are the identifiers the generator will emit for the selected widget in its current screen scope.",
      entries,
      highlightedSymbols,
    );
  }
  ...
  function styleSymbolPreviewMarkup(style, highlightedSymbols = new Set()) {
    ...
    return symbolPreviewMarkup(
      "Codegen Preview",
      "This is the normalized symbol the LVGL generator will use for the selected shared style.",
      [
  ...
  function renderField(field, context) {
    if (!window.LvglRegistry?.isFieldVisible(field, context)) return "";
    ...
    panel.querySelectorAll("[data-lvgl-style-ref]").forEach(input => {
      input.addEventListener("change", () => {
        ...
        applyMutation(() => {
          ...
          if (input.checked) refs.add(styleId);
          else refs.delete(styleId);
  ...
  function init() {
    ...
    window.$("#lvglQuickStyleBg")?.addEventListener("input", (event) => {
      ...
      if (!targets.length) {
        const selected = window.lvglSelectedNode();
        if (!selected || selected.styleMode === "shared") return;
        targets.push(selected);
      }
      applyMutation(() => {
        targets.forEach(target => {
          const current = window.lvglFindNode(target.id)?.node;
          if (!current || current.styleMode === "shared") return;
    ...
    window.addEventListener("mousemove", (event) => {
      if (!window.lvglLayoutDrag) return;
      ...
      if (window.lvglLayoutDrag.mode === "resize") {
        ...
        window.lvglLayoutDrag.origins.forEach(origin => {
          ...
          if (handle.includes("e")) metrics.w = origin.w + deltaX;
          if (handle.includes("s")) metrics.h = origin.h + deltaY;
          if (handle.includes("w")) {
    ...
    canvasWrap?.addEventListener("mousedown", (event) => {
      if (event.target.closest("[data-lvgl-node]")) return;
      if (canvasView.tool !== "hand") return;
      window.lvglLayoutDrag = {
        mode: "pan",
        startClientX: event.clientX,
