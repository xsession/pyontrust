...
enum Target {TargetDevice, TargetAnalogIn, TargetAnalogOut, TargetAnalogIO, TargetDigitalIO} ;
...
bool FApi(const char * szApi, int fResult);
void StartTimeout();
bool IsTimeout();
...
bool FCmdInteger(char* szCmd, char* szParam, char* szUnit, int* pInteger){
    ...
    if(szUnit != NULL && strlen(szCmd2) > strlen(szUnit)){
        for(int i = int(strlen(szUnit))-1; i >= 0; i--) {
        ...
        switch(target){
            ...
            case TargetAnalogIn: 
                if(FCmdNumber(szCmd, "frequency=", "Hz", &dParam) || FCmdNumber(szCmd, "freq=", "Hz", &dParam)){
                    ...
                    int cChannel = 0;
                    if(!FApi("FDwfAnalogInChannelCount", FDwfAnalogInChannelCount(hdwf, &cChannel))) return false;
...
int main(int argc, char* argv[]){
    ...
    FDwfParamSet(DwfParamOnClose, 0); // continue signal generation after close

    if(argc<=1 || FCmdMatch(argv[1], "help")){
        printf("Digilent WaveForms utility application. Version 0.1.2\n");
