"""YOLO-based object detection for webcam recordings.

Migrated from pyontrust_packages/power_test_framework/vision_object_detector.py.
Ultralytics is optional — skips gracefully when unavailable.
"""
...
def _extract_jpegs(video_path: pathlib.Path, out_dir: pathlib.Path, cfg: ObjectDetectConfig) -> list[pathlib.Path]:
    ...
    cmd = [ffmpeg, "-y", "-nostdin", "-i", str(video_path), "-vf", vf, "-q:v", "3", pattern]
    subprocess.run(
        cmd,
        check=True,
...
def analyze_video_objects(
    *,
    ...
    if not ok_ml:
        summary = {"ok": True, "skipped": True, "reason": reason, "video": str(video), "backend": "ultralytics"}
        if extra:
            ...
            for idx, img in enumerate(frames):
                ...
                for r in results:
                    ...
                    for b, c, k in zip(xyxy_list, conf_list, cls_list):
                        ...
                        detections.append({"label": label, "conf": float(c), "bbox_xyxy": [float(x) for x in b]})
                        label_counts[label] = label_counts.get(label, 0) + 1
                f.write(json.dumps({"frame": idx, "detections": detections}) + "\n")
