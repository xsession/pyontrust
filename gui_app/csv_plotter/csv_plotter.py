...
from data import find_newest_csv, read_any_csv, compute_timestamp_scale_for_df
...
class CSVPlotterApp:
    def __init__(self, root):
        ...
        # Controls (top)
        self.controls_frame = ttk.Frame(self.left_pane)

        self.button_frame = ttk.Frame(self.controls_frame)
        self.button_frame.pack(pady=(0, 6), fill='x')

        ttk.Button(self.button_frame, text="Choose CSV File", command=self.choose_file).pack(side=tk.LEFT, padx=5)
        ttk.Button(self.button_frame, text="Add Subplot", command=self.add_subplot).pack(side=tk.LEFT, padx=5)
        ttk.Button(self.button_frame, text="Plot All", command=self.plot_all).pack(side=tk.LEFT, padx=5)
        ttk.Button(self.button_frame, text="Choose Folder", command=self.choose_folder).pack(side=tk.LEFT, padx=5)
        ...
        ttk.Label(self.button_frame, text="every").pack(side=tk.LEFT, padx=(6, 2))
        try:
            ...
            newest_spin = ttk.Entry(self.button_frame, width=5, textvariable=self.auto_load_newest_period_s)
        newest_spin.pack(side=tk.LEFT)
        newest_spin.bind("<FocusOut>", lambda _e: self._on_refresh_period_changed(), add="+")
        ttk.Label(self.button_frame, text="s").pack(side=tk.LEFT, padx=(2, 8))
        ...
        try:
            ...
            reload_spin = ttk.Entry(self.button_frame, width=5, textvariable=self.auto_reload_selected_period_s)
        reload_spin.pack(side=tk.LEFT)
        reload_spin.bind("<FocusOut>", lambda _e: self._on_refresh_period_changed(), add="+")
        ttk.Label(self.button_frame, text="s").pack(side=tk.LEFT, padx=(2, 0))

        # Status bar (bottom)
        self.statusbar = ttk.Label(self.root, textvariable=self.status_var, anchor="w", relief=tk.SUNKEN)
        ...
        try:
            self.left_pane.add(self.controls_frame, weight=1)
            self.left_pane.add(self.selector_scroll_frame, weight=5)
            try:
                self.left_pane.pane(self.controls_frame, minsize=140)
                self.left_pane.pane(self.selector_scroll_frame, minsize=200)
            except Exception:
                pass
        except Exception:
        ...
        try:
            if isinstance(result, tuple) and len(result) == 7:
                ...
                if period_seconds is not None:
                    result = (*result[:6], self._format_duration(period_seconds))
        except Exception:
            pass
        try:
            cache[key] = result
    ...
    def _get_df_for_path(self, path: str) -> tuple[pd.DataFrame | None, float]:
        """Return (df, timestamp_scale_to_seconds) for a CSV, using cache when possible."""
        if not path:
            return None, 1.0
        try:
        ...
        try:
            if isinstance(self.last_loaded_file, str) and self.last_loaded_file and os.path.abspath(self.last_loaded_file) == ap:
                ...
                if isinstance(df, pd.DataFrame):
                    return df, float(getattr(self, "_timestamp_scale_to_seconds", 1.0) or 1.0)
        except Exception:
            pass
        ...
        try:
            if isinstance(cached, dict) and (cached.get("mtime") == mtime) and isinstance(cached.get("df"), pd.DataFrame):
                return cached["df"], float(cached.get("scale_to_seconds", 1.0) or 1.0)
        except Exception:
            pass
        ...
        self._df_cache[ap] = {"mtime": mtime, "df": df, "scale_to_seconds": scale}
    ...
    def _update_timestamp_scale(self) -> None:
        ...
        try:
            self._timestamp_scale_to_seconds = self._compute_timestamp_scale_for_df(getattr(self, "df", None))
        except Exception:
    ...
    def _on_span_selected(self, selector: SubplotSelector, xmin: float, xmax: float) -> None:
        try:
            ...
            if float(xmax) == float(xmin):
                return
            dur_seconds = abs(float(xmax) - float(xmin)) * float(self._timestamp_scale_to_seconds)
    ...
    def _compute_signal_metrics(self, x: pd.Series, y: pd.Series) -> tuple[str, str, str, str, str, str, str]:
    ...
    def _on_selector_canvas_configure(self, event):
        ...
        try:
            req_h = int(self.selector_inner.winfo_reqheight())
            self.selector_canvas.itemconfig(self._selector_window_id, height=max(int(event.height), req_h))
        except Exception:
            pass
    ...
    def _on_plot_canvas_configure(self, event):
        ...
        try:
            ...
            self.canvas.itemconfig(self._plot_window_id, height=max(int(event.height), req_h))
        except Exception:
            pass
    ...
    def _apply_pending_ui_sashes_now(self) -> None:
        ...
        try:
            ...
            if isinstance(lst, list) and lst:
                self.main_pane.sashpos(0, int(lst[0]))
        except Exception:
            pass
        ...
        try:
            ...
            lst = lp.get("sashpos")
            if isinstance(lst, list) and lst:
                self.left_pane.sashpos(0, int(lst[0]))
        except Exception:
            pass
    ...
    def open_folder_load_all(self) -> None:
        ...
        if folder is not None and folder.exists():
            ...
            csv_paths = [str(p.resolve()) for p in items]
    ...
    def _finish_load_file(self, path: str, new_df: pd.DataFrame | None, scale_to_seconds: float, mtime: float | None, err: Exception | None) -> None:
        try:
            ...
            try:
                ...
                self.file_path = path
                try:
                    self._timestamp_scale_to_seconds = float(scale_to_seconds or 1.0)
                except Exception:
                    self._timestamp_scale_to_seconds = 1.0
    ...
    def choose_folder(self):
        ...
        if folder_path:
            self.current_folder = folder_path
            try:
                newest_file = find_newest_csv(folder_path)
    ...
    def request_replot(self):
        ...
        if self._replot_job is not None:
            try:
                ...
                pass
        self._replot_job = self.root.after(150, self._do_replot)
        self._schedule_autosave()
