...
def _missing_deps_message(module: str, *, repo_root: pathlib.Path) -> str:
    return (
        ...
        "  .\\.venv-nicegui\\Scripts\\python sdr_module\\examples\\run_standalone.py\n"
...
def _bootstrap_venv(*, venv_dir: pathlib.Path, repo_root: pathlib.Path) -> None:
    ...
    subprocess.check_call([str(py), "-m", "pip", "install", "-e", str(repo_root / "sdr_module")])
...
def main() -> None:
    ...
    if args.bootstrap and sys.prefix == sys.base_prefix:
        ...
        try:
            ...
            # Re-run without --bootstrap to avoid loops
            ...
            print(_missing_deps_message("nicegui/pyontrust_sdr", repo_root=repo_root), file=sys.stderr)
            raise SystemExit(1)
    ...
    try:
        ...
        print(_missing_deps_message(exc.name or "pyontrust_sdr", repo_root=repo_root), file=sys.stderr)
        raise SystemExit(1)

    ui.page_title("Pyontrust SDR")
    with ui.column().classes("w-full"):
        SdrModule.mount(ui.column(), config=None)
    ui.run(title="Pyontrust SDR", reload=False)
